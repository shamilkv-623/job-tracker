...
# This ensures the 'users' table exists before any queries run
def init_db():
...
def update_user_keywords(user_id: int, keywords: str):
    with conn.session as s:
        s.execute(
            "UPDATE users SET keywords = :keywords WHERE id = :id",
...
if not st.session_state.logged_in:
    ...
    with tab2:
        with st.form("signup_form"):
            new_email = st.text_input("New Email / Username")
...
# ---------------- MAIN APP ----------------
    ...
    with col2:
        ...
        # Handle cases where keywords might be None
        current_keywords = getattr(user_data, 'keywords', "") or ""
